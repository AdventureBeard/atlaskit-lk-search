...
class ResultItem extends React.PureComponent<Props> {
  ...
  render() {
    const icon = this.props.icon ? (
      <ResultItemIcon className="ak-result-item-icon">
        {this.props.icon}
      </ResultItemIcon>
    ) : null;

    const textAfter = this.props.textAfter ? (
      <ResultItemTextAfter className="ak-result-item-text-after">
        {this.props.textAfter}
      </ResultItemTextAfter>
    ) : null;
    const after =
      this.props.textAfter || this.props.selectedIcon ? (
        <ResultItemAfterWrapper className="ak-result-item-after-wrapper">
          <ResultItemAfter
            className="ak-result-item-after"
    ...
    const wrappedCaption = this.props.caption ? (
      <ResultItemCaption className="ak-result-item-caption">
        {this.props.caption}
      </ResultItemCaption>
    ) : null;

    const wrappedSubText = this.props.subText ? (
      <ResultItemSubText className="ak-result-item-subtext">
        {this.props.subText}
      </ResultItemSubText>
    ) : null;
